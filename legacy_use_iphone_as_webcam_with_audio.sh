...
ffmpeg -rtsp_transport "${RTSP_TRANSPORT}" -fflags nobuffer -flags low_delay -i rtsp://admin:admin@${IPHONE_IP}:8554/live -map 0:v -vcodec rawvideo -pix_fmt yuyv422 -f v4l2 /dev/video10 -map 0:a -f pulse iphone
